...
<html lang="id">
    <head>
        ...
        <title>{{ $portofolio->name }} - Portofolio</title>
        <meta name="description" content="{{ $portofolio->description }}">
        <meta name="author" content="{{ $portofolio->name }}">
...
@php
$data = json_decode($portofolio->data)
@endphp
    ...
    <nav class="border-gray-200 bg-gray-900 z-40 w-full sm:py-5 py-0 sm:fixed sticky top-0">
        <div class="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
            ...
            <div class="hidden w-full md:block md:w-auto" id="navbar-default">
                ...
                <li>
                    <a href="#ovr" class="block py-2 px-3 bg-{{ $data->color }}-700 rounded md:bg-transparent md:p-0 text-white md:text-{{ $data->color }}-500" aria-current="page">Overview</a>
                </li>
                <li>
                    <a href="#edu" class="block py-2 px-3 rounded md:hover:bg-transparent md:border-0 md:p-0 text-white md:hover:text-{{ $data->color }}-500 hover:bg-gray-700 hover:text-white md:hover:bg-transparent">Education</a>
                </li>
                <li>
                    <a href="#skills" class="block py-2 px-3 rounded md:hover:bg-transparent md:border-0 md:p-0 text-white md:hover:text-{{ $data->color }}-500 hover:bg-gray-700 hover:text-white md:hover:bg-transparent">Skills</a>
                </li>
                <li>
                    <a href="#exp" class="block py-2 px-3 rounded md:hover:bg-transparent md:border-0 md:p-0 text-white md:hover:text-{{ $data->color }}-500 hover:bg-gray-700 hover:text-white md:hover:bg-transparent">Experience</a>
                </li>
                <li>
                    <a href="#por" class="block py-2 px-3 rounded md:hover:bg-transparent md:border-0 md:p-0 text-white md:hover:text-{{ $data->color }}-500 hover:bg-gray-700 hover:text-white md:hover:bg-transparent">Portofolio</a>
                </li>
                </ul>
            </div>
        </div>
    ...
    <section id="ovr" class="snap-center h-screen bg-gray-900 bg-[url('https://flowbite.s3.amazonaws.com/docs/jumbotron/hero-pattern-dark.svg')] relative overflow-hidden">
        <div class="bg-gradient-to-b to-transparent via-{{ $data->color }}-900/25 from-{{ $data->color }}-900/75 w-full h-full absolute top-0 left-0 z-0 sm:px-20 px-2.5">
            ...
            <div class="flex sm:flex-row flex-col w-full h-full sm:space-x-20 space-x-0 sm:justify-center sm:pt-0 pt-5 ">
                <div class="w-full sm:h-full h-1/2 max-w-xl px-5 relative sm:order-first order-last sm:ml-0 ">
                    <div class="sm:w-96 sm:h-96 w-64 h-64 absolute bg-{{ $data->color }}-500 rounded-full left-1/2 -translate-x-1/2 sm:top-1/4 top-1/8 z-20 blur-md mt-5"></div>
                    <div class="animate-pulse sm:w-96 sm:h-96 w-64 h-64 absolute bg-{{ $data->color }}-800 rounded-full left-1/2 -translate-x-1/2 sm:top-1/4 top-1/8 z-10 scale-150 blur-lg mt-5"></div>
                    <div style="background-image: url('{{ $portofolio->photo }}');" class=" bg-bottom bg-contain bg-no-repeat w-full h-full z-30 absolute sm:bottom-10 bottom-0 left-0"></div>
                    <div class="absolute w-full max-w-sm  bg-gradient-to-t from-{{ $data->color }}-200 to-{{ $data->color }}-100 sm:bottom-10 bottom-0 z-40 left-1/2 -translate-x-1/2 rounded-xl">
                        <div class="text-center py-3 flex items-center justify-center px-3 space-x-5">
                            <h3 class="uppercase sm:text-3xl text-2xl font-black text-{{ $data->color }}-900 w-fit">ilham</h3>
                            <div class="flex place-items-center w-fit space-x-2">
                                @foreach ($data->social_media as $media)
                                    
                                <!-- INSTAGRAM -->
                                <a href="{{ $media->link }}" target="_blank" class="sm:h-10 sm:w-10 h-8 w-8 block transition-all bg-{{ $data->color }}-800 rounded-full text-white p-1.5 hover:scale-105">
                                    @if (strtoupper($media->name) == "INSTAGRAM")
                                    ...
                                    @elseif (strtoupper($media->name) == "LINKEDIN")
                                    ...
                                    @elseif (strtoupper($media->name) == "EMAIL")
                                    ...
                                    @elseif (strtoupper($media->name) == "GITHUB")
                ...
                <div class="w-full max-w-3xl flex flex-col justify-center sm:px-20 px-5 sm:space-y-6 space-y-3 ">
                    <h2 class="text-white font-black sm:text-5xl text-4xl uppercase ">
                        <span class="bg-clip-text text-transparent bg-gradient-to-r from-{{ $data->color }}-200 via-{{ $data->color }}-100 to-{{ $data->color }}-300">
                            {{ $portofolio->name }}
                        </span>
                    </h2>
                    <h3 class="font-semibold sm:text-2xl text-xl capitalize text-{{ $data->color }}-600 w-fit px-4 py-1 relative">
                    <div class="w-full h-full absolute bg-{{ $data->color }}-50 left-0 top-0 -z-10 animate-pulse rounded"></div>
                    ...
                    <a href="https://wa.me/{{ $portofolio->contact }}" target="_blank" class="flex bg-{{ $data->color }}-200 w-fit pl-5 pr-1.5 rounded-full py-1.5 space-x-3 cursor-pointer group hover:bg-{{ $data->color }}-100 transition-all">
                        <button class="text-lg font-semibold text-{{ $data->color }}-950 group-hover:scale-105 truncate">
                            Hire Me!
                        </button>
                        <div class="sm:w-12 sm:h-12 h-8 w-8 bg-{{ $data->color }}-600 text-{{ $data->color }}-100 rounded-full grid place-items-center">
                    ...
                    <div class="h-24 flex sm:-space-x-4 -space-x-1  pt-5 relative w-fit z-40">
                        ...
                        @foreach ($data->hard_skill as $skill)
                            <div data-tooltip-target="kill-{{ $skill->name }}" data-tooltip-placement="bottom" class="sm:h-16 sm:w-16 h-10 w-10 bg-{{ $data->color }}-200 rounded-full grid place-items-center border-2 border-white relative p-1.5 hover:scale-105 transition-all cursor-pointer overflow-hidden">
                                <img src="{{ $skill->logo }}" class="grayscale" alt="{{ $skill->name }}">
                            </div>
                            <div id="kill-{{ $skill->name }}" role="tooltip" class="absolute z-10 invisible inline-block px-3 py-2 text-sm font-medium text-white transition-opacity duration-300 bg-gray-900 rounded-lg shadow-sm opacity-0 tooltip">
                                {{ $skill->name }}
                                <div class="tooltip-arrow" data-popper-arrow></div>
                            </div>
                        @endforeach

                        <div class="absolute -right-2 translate-x-full sm:h-16 sm:w-16 h-10 w-10 bg-{{ $data->color }}-200 flex rounded-full items-center justify-center space-x-1 border-2 border-white p-1.5 hover:scale-105 transition-all cursor-pointer">
    ...
    <section id="edu" class="snap-center h-full sm:py-28 py-16 bg-gray-900 bg-[url('https://flowbite.s3.amazonaws.com/docs/jumbotron/hero-pattern-dark.svg')] relative">
        <div class="bg-gradient-to-t to-transparent via-{{ $data->color }}-900/25 from-{{ $data->color }}-900/75 w-full h-full absolute top-0 left-0 z-0 sm:px-20 px-2.5 h-fit">
        </div>
        ...
        <div class="relative z-10">
            ...
            <h2 class="text-white font-black sm:text-5xl text-4xl uppercase text-center mb-8">
                <span class="bg-clip-text text-transparent bg-gradient-to-r from-{{ $data->color }}-300 via-{{ $data->color }}-100 to-{{ $data->color }}-300">
                    Education
                </span>
            </h2>
                ...
                <div class="sm:w-1/2 w-full items-center flex sm:flex-row flex-col sm:space-x-5 space-x-0 sm:space-y-0 space-y-5 p-5 bg-white/10 rounded-2xl shadow-xl ">
                    ...
                    <div class="flex-1 h-fit space-y-1 sm:text-left text-center">
                        <h2 class="text-white font-black sm:text-2xl text-xl uppercase">
                            <span class="bg-clip-text text-transparent bg-gradient-to-r from-{{ $data->color }}-400 to-{{ $data->color }}-300">
                                {{ $education->name }}
                            </span>
                        </h2>
                        <h3 class="sm:text-xl text-md font-bold text-{{ $data->color }}-100">
                            {{ $education->year }}
                        </h3>
                        <h3 class="sm:text-xl text-md font-bold text-{{ $data->color }}-300">
                            {{ $education->major }}
                        </h3>
                        ...
                        <div class="py-3">
                            <a target="_blank" href="{{ $education->link }}" class="px-3 py-2 bg-{{ $data->color }}-400 rounded-lg hover:bg-{{ $data->color }}-300 transition-all">
                                About This My Education
                            </a>
                        </div>
                    </div>
                </div>
    ...
    <section id="org" class="snap-center h-full sm:py-28 py-16 bg-gray-900 bg-[url('https://flowbite.s3.amazonaws.com/docs/jumbotron/hero-pattern-dark.svg')] relative">
        <div class="bg-gradient-to-b to-transparent via-{{ $data->color }}-900/50 from-{{ $data->color }}-900/75 w-full h-full absolute top-0 left-0 z-0 sm:px-20 px-2.5 h-fit">
        </div>
        ...
        <div class="relative z-10">
            ...
            <div class="flex flex-wrap items-center justify-center gap-4 py-5 sm:px-20 px-5">
                ...
                <div class=" w-full max-w-3xl items-center flex space-x-5 p-5 bg-transparent rounded-2xl shadow-[inset_2px_2px_16px_4px_rgb(0,0,0/0.50)] shadow-{{ $data->color }}-500/50 ">
                    <div class="sm:w-1/4 w-1/3">
                        <img src="{{ $organization->logo }}" alt="" class="rounded-xl">
                    </div>
    ...
    <section id="skills" class="snap-center h-full sm:py-28 py-16 bg-gray-900 bg-[url('https://flowbite.s3.amazonaws.com/docs/jumbotron/hero-pattern-dark.svg')] relative">
        <div class="bg-gradient-to-t to-transparent via-{{ $data->color }}-900/40 from-{{ $data->color }}-900/75 w-full h-full absolute top-0 left-0 z-0 sm:px-20 px-2.5 h-fit">
        </div>
        ...
        <div class="relative z-10 grid grid-cols-1 place-items-center">
            ...
            <div class="flex flex-wrap items-center justify-center max-w-7xl">
                @foreach ($data->hard_skill as $hard_skill)
                    
                <div class="w-full max-w-sm mx-2 mb-4 px-8 py-8 border rounded-xl border-{{ $data->color }}-300 flex items-center space-x-8">
                    <div class="bg-transparent w-28 h-28 grid grid-cols-1 place-items-center rounded-lg shadow-[inset_2px_2px_16px_4px_rgb(0,0,0/0.50)] shadow-{{ $data->color }}-500/50 rotate-45">
                        <img src="{{ $hard_skill->logo }}" alt="" class="p-5 -rotate-45 grayscale">
                    </div>
                    <div class="flex-1 text-white">
                        <h1 class="font-black text-xl">{{ $hard_skill->name }}</h1>
                        <span class="italic underline">Level : {{ $hard_skill->level }}</span>
                        <p class="text-sm mt-3">{{ $hard_skill->description }}</p>
                    </div>
                </div>
    ...
    <section id="exp" class="snap-center h-full sm:py-28 py-16 bg-gray-900 bg-[url('https://flowbite.s3.amazonaws.com/docs/jumbotron/hero-pattern-dark.svg')] relative">
        <div class="bg-gradient-to-b to-transparent via-{{ $data->color }}-900/40 from-{{ $data->color }}-900/75 w-full h-full absolute top-0 left-0 z-0 sm:px-20 px-2.5 h-fit">
        </div>
        ...
        <div class="relative z-10">
            ...
            @foreach ($data->experience as $ex)
                ...
                <div class="grid sm:grid-cols-4 grid-cols-2 sm:gap-0 gap-4 items-center border-b p-4 border rounded-2xl border-{{ $data->color }}-300">
                    ...
                    <div class="grid place-items-center">
                        <div class="bg-transparent w-36 h-36 grid grid-cols-1 place-items-center rounded-full shadow-[inset_2px_2px_16px_4px_rgb(0,0,0/0.50)] shadow-{{ $data->color }}-500/50 text-white font-bold text-lg">
                            {{ preg_replace('/[^\d\s-]/', '', $ex->year) }}
                        </div>
                    </div>
                    <div class="col-span-2 w-full p-5 bg-{{ $data->color }}-600/10 rounded-2xl shadow-xl text-white">
                        <div class="flex items-center justify-between">
                            <div class="flex-1">
                                <h4 class="font-black text-xl text-{{ $data->color }}-100">
                                    {{ $ex->company }}
                                </h4>
                            ...
                            <div class="py-4 w-fit">
                                <a href="" class="px-3 py-2 bg-{{ $data->color }}-600 rounded-lg hover:bg-{{ $data->color }}-500 transition-all">
                                    See Details
                                </a>
                            </div>
                        </div>
    ...
    <section id="por" class="snap-center h-full sm:py-28 py-16 bg-gray-900 bg-[url('https://flowbite.s3.amazonaws.com/docs/jumbotron/hero-pattern-dark.svg')] relative">
        ...
        <div class="relative z-10">
            ...
            <div class="sm:px-20 px-4 py-5 grid grid-cols-1 gap-8">
                ...
                @foreach ($data->data as $porto)
                    
                <div class="grid sm:grid-cols-2 grid-cols-1 gap-6 place-items-center">
                    <div class="{{ $loop->iteration % 2 == 0 ? 'sm:order-1 order-0' : '' }}">
                        <div class="bg-gray-800 rounded-lg pt-8 pb-2 px-2 shadow-sm shadow-{{ $data->color }}-500 relative">
                            ...
                            <img src="{{ $porto->image }}" alt="{{ $porto->name }}" class="w-full rounded">
                        </div>
                    </div>
                    <div class="{{ $loop->iteration % 2 == 0 ? 'text-right' : '' }}">
                        <div class="text-white font-bold text-2xl  uppercase">
                            <h1 class="bg-clip-text text-transparent bg-gradient-to-r from-{{ $data->color }}-300 via-{{ $data->color }}-100 to-{{ $data->color }}-300">
                                {{ $porto->name }}
                            </h1>
                        </div>
                        <span class="italic text-white">
                            Created at {{ $porto->date }}
                        </span>
                        ...
                        <div class="mt-6">
                            <a href="{{ $porto->link }}" target="_blank" class="text-white px-5 py-2 bg-{{ $data->color }}-600 rounded-lg hover:bg-{{ $data->color }}-500 transition-all">
                                See Details
                            </a>
                        </div>
                    </div>
    ...
    <footer id="con" class=" snap-center p-4 md:p-8 lg:p-10 bg-gray-900">
        <div class="mx-auto max-w-screen-xl text-center">
            ...
            <div class="flex mt-4 space-x-6 sm:justify-center sm:mt-0 mb-6">
                ...
                <a href="{{ $media->link }}" target="_blank" class="text-gray-500 hover:text-white">
                    @if (strtoupper($media->name) == "INSTAGRAM")
